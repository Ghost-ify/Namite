...
BASE_REQUESTS_PER_MIN = 60    # Target 60 requests per minute per cookie
MIN_REQUEST_INTERVAL = 0.05   # Minimum 50ms between requests
SCALE_UP_THRESHOLD = 0.95     # Success rate needed to scale up requests
SCALE_DOWN_THRESHOLD = 0.80   # Success rate below which to scale down
SCALE_FACTOR = 1.2           # How much to scale up/down by
...
class AdaptiveLearning:
    ...
    def calculate_dynamic_values(self):
        ...
        base_per_cookie = BASE_REQUESTS_PER_MIN / 60  # Requests per second

        # Adjust base per cookie based on average success rate
        if avg_success_rate > SCALE_UP_THRESHOLD:
            base_per_cookie *= SCALE_FACTOR
        elif avg_success_rate < SCALE_DOWN_THRESHOLD:
            base_per_cookie /= SCALE_FACTOR
        ...
        self.check_interval = max(MIN_REQUEST_INTERVAL, 1 / (optimal_checks * 2))  # Minimum 50ms between batches
